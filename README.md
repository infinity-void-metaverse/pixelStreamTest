...
- **Composer** — send *any* payload, in two modes. **Form** mode: add field name/value rows and the JSON is built for you, automatically wrapped in the `{ "message": … }` envelope (values auto-detect numbers, booleans and JSON; quote a value to force text). **Advanced (JSON)** mode: write the full payload yourself — valid JSON is sent as an object, anything else as a plain string, exactly as written. Both modes share a live "Will send" preview and Ctrl/Cmd+Enter to send. Switching modes carries your payload across.
